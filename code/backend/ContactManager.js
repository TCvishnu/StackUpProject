...
export default function ContactManager(){
    ...
    const [contacts, setContacts] = useState(
        ...
        const handleSubmit = (event) => {
            ...
            if(checkNumber()){
                ...
                .then((item)=>{setContacts(item.userData)})
        ...
        const deleteContact = (key) => {
           ...
           fetch("http://localhost:5000/delete",{
               ...
               body: JSON.stringify({username:"wtfhridya",index: key})
            }).then(item=>item.json())
           .then(item=>{setContacts(item.userData)})
    ...
    const editContact = (key)=>{
        fetch("http://localhost:5000/edit",{
               ...
               body: JSON.stringify({username:"wtfhridya",index: key,updatedContact: {name: "pranav",number: "9999999999",email: "gmail@email.com"}})
            }).then(item=>item.json())
           .then(item=>{setContacts(item.userData)})
    }
                    ...
                    {contacts.map((contact,index)=>{
                        return (<tr key={index} className='row'>
                            ...
                            <td className='cell text-center p-2' colSpan="1">
                                <button className='actionBtn' onClick={()=>{editContact(index)}} title='Edit Contact'>
